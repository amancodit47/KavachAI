  ...
  return <div className="mx-auto max-w-[1440px]">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <section className="k-card k-reveal k-reveal-3 p-5 md:p-6">
        ...
        {activityQuery.isLoading ? <div className="grid gap-4">{[1, 2, 3, 4].map((item) => <Skeleton className="h-10" key={item} />)}</div> : activityQuery.isError ? <ErrorState onRetry={() => activityQuery.refetch()} /> : Array.isArray(activityQuery.data) && activityQuery.data.length ? <ActivityList events={activityQuery.data.slice(0, 6)} /> : <EmptyState icon={Activity} title="No agent events" detail="Activity will appear as agents inspect, reproduce, and verify." />}
      </section>
    </div>
...
function AgentTrail() {
  const activityQuery = useListActivity();
  const events = Array.isArray(activityQuery.data) ? activityQuery.data : [];
  return <div className="mx-auto max-w-[1100px]"><PageHeader eyebrow="Proof system / 05" title="Agent trail" detail="Technical events are the audit surface: what was observed, when it happened, and which run owns it." action={<button className="k-button k-button-ghost k-focus" onClick={() => activityQuery.refetch()} data-testid="button-refresh-activity"><RotateCcw size={14} /> Refresh trail</button>} /><section className="k-card p-5 md:p-7"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="k-eyebrow">Event stream</p><p className="mt-1 text-sm">Backend activity feed · timestamps in local display</p></div><span className="k-pill k-status-muted">structured events only</span></div>{activityQuery.isLoading ? <div className="grid gap-4">{[1, 2, 3, 4, 5].map((item) => <Skeleton className="h-12" key={item} />)}</div> : activityQuery.isError ? <ErrorState onRetry={() => activityQuery.refetch()} /> : events.length ? <div className="divide-y divide-[hsl(var(--border))]">{events.map((event) => <div className="grid gap-3 py-4 first:pt-0 md:grid-cols-[130px_1fr_auto]" key={event.id} data-testid={`trail-event-${event.id}`}><span className="k-mono text-[10px] text-[hsl(var(--muted-foreground))]">{formatDate(event.createdAt)}</span><div><p className="text-sm font-medium">{event.message}</p><p className="mt-1 text-xs leading-5 text-[hsl(var(--muted-foreground))]">{event.detail}</p></div><span className="k-pill k-status-muted h-fit justify-self-start">{event.kind}</span></div>)}</div> : <EmptyState icon={Activity} title="No events recorded" detail="A scan will populate the technical event trail." />}</section></div>;
}
